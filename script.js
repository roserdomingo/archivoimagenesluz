...
// Mostrar o ocultar hamburguesa según scroll

let lastGalleryScroll = 0;
let lastMenuScroll = 0;

function hideToggle() {
  mobileMonthToggle.style.opacity = '0';
  mobileMonthToggle.style.pointerEvents = 'none';
}

function showToggle() {
  mobileMonthToggle.style.opacity = '1';
  mobileMonthToggle.style.pointerEvents = 'auto';
}

//Scroll en la galería
window.addEventListener('scroll', () => {
  //Si el menú está abierto, ignoramos el scroll de la galería
  if (monthFilter.classList.contains('open')) return;

  const currentScroll = window.scrollY;

  if (currentScroll > lastGalleryScroll) {
    hideToggle(); //Bajar
  } else {
    showToggle(); //Subir
  }

  lastGalleryScroll = currentScroll <= 0 ? 0 : currentScroll;
});

//Scroll dentro del menú
monthFilter.addEventListener('scroll', () => {
  const currentScroll = monthFilter.scrollTop;

  if (currentScroll > lastMenuScroll) {
    hideToggle(); //Bajar
  } else {
    showToggle(); //Subir
  }

  lastMenuScroll = currentScroll <= 0 ? 0 : currentScroll;
});